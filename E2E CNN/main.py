...
from dataloader import ProjData2
from model_configs import dataset, device
from model import RecModel, CustomModel
import torch
import numpy as np
import time
...
def run_train(dataset, model, optimiser):
    model.train()
    num_batches = dataset.n_user // BATCH_SIZE + 1
    mean_batch_loss = 0
    ...
    t = tqdm(enumerate(utils.minibatch((users,posItems,negItems), BATCH_SIZE)), total=num_batches, desc='Desc', leave=True)
    for i,(b_users, b_pos, b_neg) in t:
    #for i,(b_users, b_pos, b_neg) in enumerate(utils.minibatch((users,posItems,negItems), BATCH_SIZE)):
        ...
        mean_batch_loss += loss.detach().cpu().item()

        loss.backward()
        optimiser.step()
        t.set_description(f"loss = {loss.cpu().item():.8f}")
        #print(f"Batch {i+1}/{num_batches}: loss = {loss.cpu().item():.8f}" + " " * 20)
...
def load_and_eval():
    latent_dim=256
    PATH = ''
    model = RecModel(dataset, latent_dim).to(device)
    model.load_state_dict(torch.load(PATH))
    # Precompute img embeds
    # 
...
def proj2():
    ...
    for epoch in range(EPOCHS):
        run_train(dataset, model, optimiser)
